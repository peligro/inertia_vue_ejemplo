<script setup>
import { useForm } from '@inertiajs/vue3';
import TextInput from '../Components/TextInput.vue';

let form = useForm({
    name: null,
    email: null,
    password: null,
    password_confirmation: null,
    avatar:null,
    preview:null
});
let submit = () => {
    //form.post('/clientes/tamila/test/laravel/inertia_vue/public/register');
    form.post(route('ruta_register_post'),
        {
            onError: () => form.reset('password', 'password_confirmation')
        });
};
let change=(e)=>
{
    console.log(e.target.files[0]);
    form.avatar=e.target.files[0];
    form.preview = URL.createObjectURL(e.target.files[0])
};
/*
import {reactive} from 'vue';
import { router } from '@inertiajs/vue3'
let form =reactive({
    name:null,
    email:null,
    password:null,
    password_confirmation:null
});
let submit=()=>
{
    console.log("submit "+form);
    //router.post('/register', form);
    router.post('/clientes/tamila/test/laravel/inertia_vue/public/register', form);
};
*/
</script>
<template>

    <Head title="Register" />
    <h1 class="title">Register a new account</h1>
    <div class="w-2/4 mx-auto">
        <form @submit.prevent="submit()">
            <!-- <input type="file" @input="form.avatar = $event.target.files[0]" />-->
            <!--<div>
                <label for="avatar">Avatar</label>
                <input type="file" id="avatar" @input="change" />

                 <p>{{ form.errors.avatar }}</p>
            </div>
        -->
         <!-- Upload Avatar -->
      <div class="grid place-items-center">
        <div
          class="relative w-28 h-28 rounded-full overflow-hidden border border-slate-300"
        >
          <label for="avatar" class="absolute inset-0 grid content-end cursor-pointer">
            <span class="bg-white/70 pb-2 text-center">Avatar</span>
          </label>
          <input type="file" @input="change" id="avatar" hidden />

          <img
            class="object-cover w-28 h-28"
            :src="form.preview ?? 'img/default.png'"
          />
        </div>

        <p class="error mt-2">{{ form.errors.avatar }}</p>
      </div>
      <!-- End Upload Avatar -->

            <TextInput name="name" v-model="form.name" :message="form.errors.name" />

            <TextInput name="email" type="email" v-model="form.email" :message="form.errors.email" />

            <TextInput name="password" type="password" v-model="form.password" :message="form.errors.password" />

            <TextInput name="confirm password" type="password" v-model="form.password_confirmation" />
            <!--
            <div class="mb-6">
                <label>Name</label>
                <input type="text" v-model="form.name" />
                <small>{{ form.errors.name }}</small>
            </div>

            <div class="mb-6">
                <label>E-Mail</label>
                <input type="text" v-model="form.email" value="{{ old('email') }}" />
                <small>{{ form.errors.email }}</small>
            </div>
            <div class="mb-6">
                <label>Password</label>
                <input type="password" v-model="form.password" />
                <small>{{ form.errors.password }}</small>
            </div>
            <div class="mb-6">
                <label>Confirm password</label>
                <input type="password" v-model="form.password_confirmation" />
                <small>{{ form.errors.password_confirmation }}</small>
            </div>
            -->
            <div>
                <p class="text-slate-600 mb-2">Already a user? <a :href="route('ruta_login')" class="text-link">Login</a></p>
                <button class="primary-btn" :disabled="form.processing">Register</button>
            </div>
        </form>
    </div>
</template>
